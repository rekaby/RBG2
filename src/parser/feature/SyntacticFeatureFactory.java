...
public class SyntacticFeatureFactory implements Serializable {
    ...
    public FeatureVector createArcFeatures(DependencyInstance inst, int h, int c) 
    {
    	...
    	if(options.context)
    	{
    		...
    		addArcContextFeatures(fv, inst, h, c, attDist,-1);// -1 as default value which means no type
    		if(fv.size()>old)
    ...
    {
    	addArcContextAgentFeatures(fv, inst, h, m, attDist,type);
    	addArcContextThemeFeatures(fv, inst, h, m, attDist,type);
    	addArcContextInstrumentFeatures(fv, inst, h, m, attDist,type);
    	addArcContextOwnerFeatures(fv, inst, h, m, attDist,type);
    }
   ...
   /* 	code = createArcCodeWW(HW_MAGW, forms[head], forms[mod]);
    ...
    public void addCore1OBigramInstrumentFeatures(FeatureVector fv, DependencyInstance inst, int head, int headP, 
    		int mod, int modP, int attDist, int type) 
    {
   ...
   /* 	code = createArcCodeWW(HW_MAGW, forms[head], forms[mod]);
    ...
    public void addCore1OBigramOwnerFeatures(FeatureVector fv, DependencyInstance inst, int head, int headP, 
    		int mod, int modP, int attDist, int type) 
    {
   ...
   /* 	code = createArcCodeWW(HW_MAGW, forms[head], forms[mod]);
    ...
    public FeatureVector createLabeledArcFeatures(DependencyInstance inst, int h, int c, int type) 
    {
    	...
    	if(options.context)
    	{
    		...
    		addArcContextFeatures(fv, inst, h, c, attDist,type);
    		if(fv.size()>old)
    			{
    				//System.out.println("==========C="+c+ " H="+h+"="+fv.size()+ " - "+old);
    			}
